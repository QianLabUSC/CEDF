...
def gauss(mean, scale, sigma, k, x=np.linspace(-10,9,20)):
...
result_number = np.linspace(0, 9, 10)
for k in [0.2, 0.4, 0.6, 0.8, 1]:
    for sigma in [0.5,1,1.5,2,2.5]:
        name = 'k' + str(k) + 'sigma' + str(sigma)
        plt.figure()
        plt.bar(result_number + 0.5, result, width = 0.4)
        plt.title('Example for Variable coverage' + name, fontsize=40)
        plt.xlabel('variable')
        plt.ylabel('sample numbers')

        information_converage = np.zeros((10))
        for i in range(len(result_number)):
            single_converage = gauss(result_number[i] +0.5,result[i], sigma, k)
            information_converage += single_converage
            plt.plot(result_number, single_converage,'go')
            plt.plot(result_number, single_converage,'g')
        plt.plot(result_number, information_converage, 'ro')
        plt.plot(result_number, information_converage, 'r')
