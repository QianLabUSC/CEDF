...
plt.figure(1000+10* i)
fig, axs = plt.subplots(5, 1, sharex=True)
for i in [0,0.2,0.4,0.6,0.8]:

    axs[int(5*i)].scatter(information_level_set[i][0], discrepancy_level_set[i][0], s=200, c=colormap[0], marker='H', label='hypo00')
    axs[int(5*i)].scatter(information_level_set[i][1], discrepancy_level_set[i][1], s=200, c=colormap[0.2], marker='H', label='hypo01')
    axs[int(5*i)].scatter(information_level_set[i][2], discrepancy_level_set[i][2], s=200, c=colormap[0.4], marker='H', label='hypo10')
    axs[int(5*i)].scatter(information_level_set[i][3], discrepancy_level_set[i][3], s=200, c=colormap[0.6], marker='H', label='hypo11')
    axs[int(5*i)].legend()
    axs[int(5*i)].set_ylabel('discrepancy')
    axs[int(5*i)].set_xlabel('information')
    axs[int(5*i)].set_title('confidence driven mode')
    axs[int(5*i)].set_ylim((0,1))
    axs[int(5*i)].set_xlim((0,1))
...
for i in [0.2,0.4,0.6,0.8]:
    ...
    plt.xlabel('confidence')
    plt.title('fitting error vs confidence')
    plt.xlim((-0.15,1))
plt.savefig('fittingerror vs confidece')
